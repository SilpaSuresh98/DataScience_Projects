#  Data Science Projects Portfolio

Welcome to my Data Science portfolio!  
This repository contains a collection of my end-to-end Data Science projects and learning experiments.  
Each project demonstrates real-world data handling — from data cleaning and analysis to machine learning and visualization.

---

## Folder Overview

| Category | Description |
|-----------|--------------|
| **Regression** | Predicting continuous outcomes using Linear Regression, Multiple Regression, and other algorithms |
| **Classification** | Binary and multi-class classification models using Logistic Regression, Decision Trees, Random Forest, etc. |
| **Clustering** | Unsupervised learning techniques such as K-Means and Gaussian Mixture Models |
| **Deeplearning** | Useed MLP and LSTM for classify whether a person is running or walking based on the given predictor variables. |
| **INX_Employee_Performance_Project(IABAC)** |A data-driven system to help INX identify department-wise performance, key influencing factors, and predict employee performance for better HR decisions.|


---


##  Project Highlights

- **Regression:** Predicting house prices using Linear Regression  
- **Classification:** Customer churn prediction with Decision Trees  
- **Clustering:** FIFA 20 player segmentation using K-Means
- **DeepLearning:** Walk& run classification using mlp & LSTM.
- **INX_Employee_Performance_Project:** Employee classification using Boosting and SVM models(IABAC project).

Each project folder includes:
- Cleaned datasets or data sources  
- Jupyter notebooks with full analysis & model development  
- Visualizations and performance metrics  

---

## Purpose

This repository showcases my Data Science learning journey — from foundational data analysis to advanced model development.  
It also serves as my **portfolio** for career opportunities in the field of **Data Science & Machine Learning**.

